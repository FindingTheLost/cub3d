...
*/
static char	*t_anim_frame_path(char *path, int frame)
{
	char	*frame_number;
	char	*file_name;
...
static int	t_anim_mlx_to_image(t_image **frame, char *path, void *mlx)
{
	int		width;
	int		height;
	void	*texture;
...
*/
static int	t_anim_validate_frames(t_anim **anim_ref, int path_dir, void *mlx)
{
	int		i;
	char	dir[3];
	char	*path;
	...
	while (i < (*anim_ref)->frame_count)
	{
		frame_path = t_anim_frame_path(path, (i + 1));
		if (!frame_path)
			return (free(path), false);
		if (!check_texture(frame_path))
			return (free(path), free(frame_path), false);
		if (!t_anim_mlx_to_image(&(*anim_ref)->frames[i], frame_path, mlx))
			return (free(path), free(frame_path), false);
		free(frame_path);
		i++;
	}
...
int	t_anim_init_texture(t_anim **anim, int width, int height, void *mlx)
{
	...
	if (!t_anim_resolution_directory(&path_dir, width, height))
		return (false);
	if (!t_anim_validate_frames(anim, path_dir, mlx))
		return (false);
	return (true);
}
