...
void	t_game_draw_transparent(t_game *g, t_image *frame)
{
	...
	while (y < frame->height)
	{
		...
		while (x < frame->width)
		{
			...
			{
				buffer = g->image->address + (y * g->image->line_length)
					+ (x * (frame->bpp / 8));
				*(int *)buffer = color;
			}
			x++;
	...
	mlx_put_image_to_window(g->mlx, g->mlx_window, g->image->image, 0, 0);
}
